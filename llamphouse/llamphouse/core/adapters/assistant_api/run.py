...
# Map internal (protocol-agnostic) event names → OpenAI SSE event names.
# The AssistantAPI adapter is intentionally OpenAI-compatible, so it
# translates before emitting to external clients.
_INTERNAL_TO_OPENAI_EVENT = {
    "run.created":        "thread.run.created",
    "run.queued":         "thread.run.queued",
    "run.in_progress":    "thread.run.in_progress",
    "run.awaiting_tools": "thread.run.requires_action",
    "run.completed":      "thread.run.completed",
    "run.failed":         "thread.run.failed",
    "run.cancelling":     "thread.run.cancelling",
    "run.cancelled":      "thread.run.cancelled",
    "run.expired":        "thread.run.expired",
    "run.step.created":   "thread.run.step.created",
    "run.step.in_progress": "thread.run.step.in_progress",
    "run.step.delta":     "thread.run.step.delta",
    "run.step.completed": "thread.run.step.completed",
    "run.step.failed":    "thread.run.step.failed",
    "run.step.cancelled": "thread.run.step.cancelled",
    "run.step.expired":   "thread.run.step.expired",
    "message.created":    "thread.message.created",
    "message.in_progress": "thread.message.in_progress",
    "message.delta":      "thread.message.delta",
    "message.completed":  "thread.message.completed",
    "message.incomplete": "thread.message.incomplete",
}
            ...
            if request.stream:
                ...
                if task_key not in req.app.state.event_queues:
                    req.app.state.event_queues[task_key] = req.app.state.queue_class(
                        assistant_id=request.assistant_id, thread_id=thread_id,
                    )
                    ...
                    while True:
                        try:
                            ...
                            if event is None:  # Stream completion signal
                                break
                            openai_name = _INTERNAL_TO_OPENAI_EVENT.get(event.event, event.event)
                            yield Event(event=openai_name, data=event.data).to_sse()
...
def get_assistant_by_id(assistants: List[Agent], assistant_id: str) -> Agent:
...
async def handle_stream_event(event: dict, db: BaseDataStore, thread_id: str, request: RunCreateRequest, assistant: Agent):
    evt = event.get("event")
    ...
    if evt == event_type.THREAD_CREATED:
        ...
        status_map = {
            ...
            event_type.RUN_REQUIRES_ACTION: run_status.AWAITING_TOOLS,
