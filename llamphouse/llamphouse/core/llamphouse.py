...
______[===]______{_R}"""
        ...
        webhook_triggers = [
            (agent, t)
            for agent in self.agents
            for t in getattr(agent, "triggers", [])
            if isinstance(t, WebhookTrigger)
        ]
        if webhook_triggers:
            llamphouse_logger.info(f"  {_DIM}Triggers ({len(webhook_triggers)}){_R}")
            for agent, trigger in webhook_triggers:
                path = "/" + trigger.path
