...
class AsyncWorker(BaseWorker):
    ...
    async def process_run_queue(self, data_store: BaseDataStore, run_queue: BaseQueue, assistants: Sequence[Agent], fastapi_state):
        ...
        while self._running:
            try:
                ...
                # Fire each run as a concurrent task so the dequeue loop is never
                # blocked by a long-running assistant.run() call.  This is what
                # allows one agent to call another agent on the same server via A2A
                # without deadlocking: while task A awaits a specialist run to
                # complete, the dequeue loop picks up that specialist run immediately.
                asyncio.create_task(
                    self._execute_run(item, data_store, run_queue, assistants, fastapi_state)
                )
    ...
    async def _execute_run(self, item: Tuple[str, QueueMessage], data_store: BaseDataStore, run_queue: BaseQueue, assistants: Sequence[Assistant], fastapi_state):
        run_object = None
        output_queue = None
        token = None
        ...
        try:
            with span_context(
                ...
                try:
                    ...
                    # Internal dispatch (call_agent / handover) registers
                    # the queue under the run_id itself; external adapters
                    # (A2A, AssistantAPI) use "assistant_id:thread_id".
                    output_queue = (
                        fastapi_state.event_queues.get(run_id)
                        or fastapi_state.event_queues.get(f"{assistant_id}:{thread_id}")
                    )
                    ...
                    span.set_attribute("assistant.name", getattr(assistant, "name", assistant_id))
                    ...
                    context = await Context.create(
                        ...
                        traceparent=carrier,
                        run_queue=run_queue,
                        event_queues=fastapi_state.event_queues,
                        queue_class=fastapi_state.queue_class,
